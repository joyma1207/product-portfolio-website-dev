...
const THREE_IMAGES_LAYOUT_IDS = ["aritzia"] as const;
...
export function ProjectCard({ project }: Props) {
  ...
  const isStorytellerGrid =
    project.id === STORYTELLER_GRID_ID && imageMedia.length >= 5;
  ...
  return (
    ...
    >
      <div className="group relative z-10 flex h-full w-full flex-col text-left rounded-[30px] min-h-0">
        ...
        >
          <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-baseline justify-start sm:justify-between gap-y-1 gap-x-3">
            <h3 className="text-xl font-bold text-gray-900 tracking-brand">{project.company}</h3>
            {project.cardPill && (
